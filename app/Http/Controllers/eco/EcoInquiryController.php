...
class EcoInquiryController extends Controller
{
    ...
    public function issueQuotation(Request $request, Inquiry $inquiry)
    {
        $validated = $request->validate([
            ...
            'items.*.price' => 'required|numeric|min:0', // This is now treated as the Item Total
            'delivery_date' => 'nullable|date',
            'payment_terms' => 'required|string',
            'notes' => 'nullable|string',
        ]);

        // Sum up the prices directly as provided in the UI
        $grandTotal = collect($validated['items'])->sum('price');
