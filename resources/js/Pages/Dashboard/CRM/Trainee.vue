...
// Actions
const submitGrade = () => {
    // Validate all criteria are set
    if (criteria.some(c => gradeForm.value[c.id] === 0)) {
        triggerToast('Please rate all criteria before submitting.');
        return;
    }
    router.post(route('crm.trainee.grade', selectedTrainee.value.id), gradeForm.value, {
        preserveScroll: true,
        onSuccess: () => {
            triggerToast(`Grade saved for ${selectedTrainee.value.name}. Score: ${gradeTotal.value.toFixed(1)}%`);
            closeModals();
        },
        onError: (errors) => {
...
const passTrainee = () => {
    router.post(route('crm.trainee.pass', selectedTrainee.value.id), {}, {
        preserveScroll: true,
        onSuccess: () => {
        ...
        onError: (errors) => {
            triggerToast(Object.values(errors)[0] || 'Failed to promote trainee.');
        }
    });
};
...
const failTrainee = () => {
    ...
    router.post(route('crm.trainee.fail', selectedTrainee.value.id), {
        reason: failReason.value
    }, {
        preserveScroll: true,
        onSuccess: () => {
            triggerToast(`${selectedTrainee.value.name} failed training and archived.`);
            closeModals();
        },
        onError: (errors) => {
...
<template>
    <Head title="CRM Trainee Management" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 sm:space-y-8">
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    CRM Trainee <span class="text-blue-600">Development</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">
                    Monitor and manage trainees assigned to CRM department.
                </p>
            </div>
            ...
            <div v-if="trainees.length === 0" class="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 p-12 text-center">
                ...
                <p class="text-sm text-slate-500 mt-2">When applicants pass the interview for CRM department, they will appear here as trainees.</p>
            </div>

            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div v-for="trainee in trainees" :key="trainee.id" class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden hover:shadow-lg transition-all">
                    <div class="p-6">
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-4">
                                <div class="h-14 w-14 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 flex items-center justify-center text-xl font-black">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs font-bold text-blue-600 mt-1">CRM Department</p>
                                </div>
                            </div>
                        ...
                        <div class="mt-6 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-xl">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <p :class="['text-sm font-black', getGradeColor(trainee.grade_percentage)]">
                            ...
                            <div class="w-full bg-slate-200 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-500"
                                    :class="[
                                        trainee.grade_percentage >= 80 ? 'bg-emerald-500' :
                                        trainee.grade_percentage >= 60 ? 'bg-blue-500' :
                                        trainee.grade_percentage >= 40 ? 'bg-amber-500' : 'bg-red-500'
                                    ]"
                        ...
                        <div class="mt-6 flex flex-wrap gap-2">
                            <button @click="openGradeModal(trainee)" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-amber-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-amber-700 transition-all">
                                <Star class="h-4 w-4" /> Grade
                            </button>
                            <button v-if="trainee.grade_percentage >= 80" @click="openPassModal(trainee)" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-emerald-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-emerald-700 transition-all">
                                <UserCheck class="h-4 w-4" /> Pass to Staff
                            </button>
                            <button v-else @click="openFailModal(trainee)" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-red-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-red-700 transition-all">
                                <UserMinus class="h-4 w-4" /> Fail
                            </button>
                        </div>
                        <p v-if="trainee.grade_percentage < 80" class="mt-3 text-xs text-center text-amber-600 font-medium">
                            Need at least 80% to be eligible for promotion.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div v-if="isGradeModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" @click.self="closeModals">
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-6">
                    <div v-for="criterion in criteria" :key="criterion.id">
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">{{ criterion.label }}</p>
                        <div class="flex gap-2">
                            <button v-for="star in 5" :key="star" type="button" @click="gradeForm[criterion.id] = star" class="focus:outline-none transition-transform active:scale-110">
                                <Star :class="[
                    ...
                    <div v-if="gradeTotal > 0" class="mt-6 p-4 rounded-2xl border-2 transition-all" :class="isPassing ? 'border-emerald-300 bg-emerald-50 dark:bg-emerald-900/20' : 'border-red-200 bg-red-50 dark:bg-red-900/20'">
                        <div class="flex justify-between items-center">
                            <div>
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Average</p>
                                <p :class="['text-3xl font-black mt-0.5', isPassing ? 'text-emerald-600' : 'text-red-500']">
                                    {{ gradeTotal.toFixed(1) }}%
                                </p>
                            </div>
                            <div v-if="isPassing" class="flex items-center gap-2 px-3 py-2 bg-emerald-100 text-emerald-700 rounded-xl">
                        ...
                        <p v-if="isPassing" class="text-xs text-emerald-600 mt-2 font-medium">✓ This trainee will automatically be eligible for promotion to Staff upon saving.</p>
                    </div>

                    <div class="flex gap-3 pt-4">
                        <button @click="closeModals" class="flex-1 py-3 text-slate-500 font-bold text-xs uppercase">Cancel</button>
                        <button @click="submitGrade" :disabled="!gradeTotal" class="flex-1 py-3 bg-amber-600 text-white rounded-xl text-xs font-black uppercase shadow-lg hover:bg-amber-700 transition-all disabled:opacity-50">
                            Save Grades
                        </button>
                    </div>
                </div>
        ...
        <div v-if="isPassModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" @click.self="closeModals">
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden text-center">
                ...
                <div class="p-6">
                    <p class="text-slate-600 dark:text-slate-300 mb-6">Are you sure you want to promote <strong>{{ selectedTrainee?.name }}</strong> to official staff? They will gain full employee access.</p>
                    <div class="flex gap-3">
                        <button @click="closeModals" class="flex-1 py-3 text-slate-500 font-bold text-xs uppercase">Cancel</button>
                        <button @click="passTrainee" class="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase shadow-lg hover:bg-emerald-700 transition-all">Confirm Promotion</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <textarea v-model="failReason" rows="3" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-red-500" placeholder="e.g., Failed to meet performance standards..."></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button @click="closeModals" class="flex-1 py-3 text-slate-500 font-bold text-xs uppercase">Cancel</button>
                        <button @click="failTrainee" :disabled="!failReason.trim()" class="flex-1 py-3 bg-red-600 text-white rounded-xl text-xs font-black uppercase shadow-lg hover:bg-red-700 transition-all disabled:opacity-50">Confirm Fail</button>
                    </div>
                </div>
            </div>
        </div>
