...
const failApplicant = () => {
    ...
    router.post(route('hrm.interview.fail', selectedApplicant.value.id), {
    ...
    <AuthenticatedLayout>
        ...
        <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 sm:space-y-8">
            ...
            <div>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">
                    Manage candidates assigned to your department for initial interview.
                </p>
            </div>
            ...
            <div v-if="applicants.length === 0"
                ...
                <h3 class="text-lg font-bold text-slate-600 dark:text-slate-400">No applicants assigned for interview
                </h3>
                <p class="text-sm text-slate-500 mt-2">When HR accepts an applicant and assigns them to your department,
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center gap-4">
                                <div
                                    class="h-14 w-14 rounded-2xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 flex items-center justify-center text-xl font-black">
                                    {{ getInitials(applicant.name) }}
                                </div>
                                <div>
                                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ applicant.name }}
                                    </h3>
                            ...
                            <div v-if="applicant.scheduled_at" class="text-right">
                                <span
                                    class="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">Scheduled</span>
                            </div>
                        </div>

                        <!-- Interview details if scheduled -->
                        <div v-if="applicant.scheduled_at"
                            class="mt-4 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-xl space-y-2">
                            <div class="flex items-center gap-2 text-sm">
                                <Calendar class="h-4 w-4 text-blue-500" />
                                <span class="font-medium">{{ formatDateTime(applicant.scheduled_at) }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-sm">
                                <component :is="getInterviewTypeIcon(applicant.interview_type)"
                                    class="h-4 w-4 text-purple-500" />
                                <span class="capitalize">{{ applicant.interview_type }}</span>
                                <span v-if="applicant.duration" class="text-slate-400">({{ applicant.duration }}
                                    min)</span>
                            </div>
                            ...
                            <div v-if="applicant.notes"
                                class="text-sm text-slate-500 mt-2 border-t border-slate-200 pt-2">
                        ...
                        <div class="mt-6 flex flex-wrap gap-2">
                            <button @click="openScheduleModal(applicant)"
                                class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-blue-700 transition-all">
                                <Calendar class="h-4 w-4" /> Schedule
                            </button>
                            <button @click="openPassModal(applicant)"
                            ...
                            <button @click="openFailModal(applicant)"
                            ...
                            <button @click="openPassToOtherModal(applicant)"
                                class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-purple-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-purple-700 transition-all">
        ...
        <div v-if="isScheduleModalOpen"
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Date &
                            Time *</label>
                        <input type="datetime-local" v-model="scheduleForm.scheduled_at"
                            class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-blue-500"
                            required />
                    </div>
                    <div>
                        <label
                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Interview
                            Type *</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label
                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Duration
                                (min)</label>
                            <select v-model="scheduleForm.duration"
                                class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                    ...
                    <div>
                        <label
                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Interviewer(s)</label>
                        <input type="text" v-model="scheduleForm.interviewer"
                            class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                            placeholder="Name of interviewer(s)" />
                    </div>
                    <div>
                        <label
                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Notes</label>
                    ...
                    <div class="flex gap-3 pt-4">
                        ...
                        <button @click="scheduleInterview"
                            class="flex-1 py-3 bg-blue-600 text-white rounded-xl text-xs font-black uppercase shadow-lg hover:bg-blue-700 transition-all">Confirm
                            Schedule</button>
                    </div>
                </div>
            </div>
        ...
        <div v-if="isPassModalOpen"
            ...
            <div
                ...
                <div class="p-6">
                    <p class="text-slate-600 dark:text-slate-300 mb-6">Are you sure you want to pass <strong>{{
                            selectedApplicant?.name }}</strong>? This will convert them into a trainee of your
                        department.</p>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <textarea v-model="failReason" rows="3"
                            ...
                            placeholder="e.g., Insufficient technical skills..."></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button @click="closeModals"
                            class="flex-1 py-3 text-slate-500 font-bold text-xs uppercase">Cancel</button>
        ...
        <div v-if="isPassToOtherModalOpen"
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md"
            @click.self="closeModals">
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Select
                            Module *</label>
                        <select v-model="otherModule"
                    ...
                    <div class="flex gap-3">
                        ...
                        <button @click="passToOtherModule" :disabled="!otherModule"
                            class="flex-1 py-3 bg-purple-600 text-white rounded-xl text-xs font-black uppercase shadow-lg hover:bg-purple-700 transition-all disabled:opacity-50">Confirm</button>
                    </div>
                </div>
            </div>
        </div>
